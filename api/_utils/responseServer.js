...
module.exports = (response, code, data = {}) => {
  ...
  Object.keys(defaultHeader).map(headerProps =>
    response.setHeader(headerProps, defaultHeader[headerProps])
  );

  if (data.serverHeader) {
    Object.keys(data.serverHeader).map(headerProps =>
      response.setHeader(headerProps, data.serverHeader[headerProps])
    );
  }
  ...
  return response.status(code).json(httpCode[code]);
};
